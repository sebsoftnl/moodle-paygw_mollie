 ...
 */
class EachPromise implements \_PhpScoper3234cdc49fbb\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    /** @var \Iterator|null */
    private $iterable;
    ...
    {
        $this->iterable = \_PhpScoper3234cdc49fbb\GuzzleHttp\Promise\Create::iterFor($iterable);
    ...
    {
        $this->mutex = \false;
        $this->aggregate = new \_PhpScoper3234cdc49fbb\GuzzleHttp\Promise\Promise(function () {
            \reset($this->pending);
            ...
            while ($promise = \current($this->pending)) {
                \next($this->pending);
                $promise->wait();
                if (\_PhpScoper3234cdc49fbb\GuzzleHttp\Promise\Is::settled($this->aggregate)) {
                    return;
                }
            }
        });
    ...
    private function addPending()
    {
        ...
        $promise = \_PhpScoper3234cdc49fbb\GuzzleHttp\Promise\Create::promiseFor($this->iterable->current());
